...
def eval(model_name, X_train, Y_train, X_test, Y_test, cnn=False, modify=None):
    ...
    # Craft trainset adversarial examples
    X_train_adv, = batch_eval([x], [adv_x], [X_train], {"batch_size": 128})

    new_model = model_from_json(open("models/{}.json".format(model_name)).read(),
                               {'RBFLayer':RBFLayer, 'InitFromFile':InitFromFile})
    new_model.build(x_shape)
    new_predictions = new_model(x)
    ...
    X_train_aug = np.concatenate((X_train_adv, X_train))
    ...
    # craft test adversarial examples again
    adv_x = fgsm(x, new_predictions, eps=0.3)
    X_test_adv, = batch_eval([x], [adv_x], [X_test], { "batch_size" : 128 })

    # accuracy on original test examples
    accuracy = th_model_eval(x, y, new_predictions, X_test, Y_test, { "batch_size" : 128 })
    print('New test accuracy on legitimate test examples: ' + str(accuracy))

    # Evaluate the accuracy of the model on adversarial examples
    accuracy = th_model_eval(x, y, new_predictions, X_test_adv, Y_test, { "batch_size" : 128 })
    print('New test accuracy on adversarial examples: ' + str(accuracy)) 
